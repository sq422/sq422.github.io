...
- （1.）HashMap底层的hash逻辑是通过hash值与容量长度减一的值做二进制位运算（hash & (length - 1)）来截取hash值的低位而非模运算。采取位运算能够让CPU执行效率更高。而长度为2的幂次方，则是为了让长度减一的二进制低位全是1，这样能让哈希分布更均匀。
...
- （2.）在进行数据迁移时，新的下标只有“不变和原下标+原容量”两种可能，数据迁移效率极高。
    - > 因为扩容后会多出一位高位，在重新进行hash时如果hash值高位为0，那么下标就不变，如果高位为1，下标就新增原来的长度即可。
...
- （2.）JDK1.8之后，HashMap的底层是数组+链表+红黑树，现在的hashmap在JDK1.7之前的版本上新增了一个机制，就是当哈希槽中的元素大于等于8个且哈希表数组的长度大于等于64个的时候会将链表转换为红黑树。如果后续红黑树结点数小于6则退化回链表。
- （3.）为什么是8：
...
- （4.）为什么要同时满足大于等于64：
        ...
        - 原因在于当数组中的容量不大的时候，扩容就能够很好地打散链表减少哈希冲突了，比起转树更简单高效且节省内存。
## *14.HashMap的扩容原理*
